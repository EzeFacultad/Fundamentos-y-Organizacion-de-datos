...
var
  a: archivo_empleado;
  numEmpleado, pos, ultimaPos: integer;
  r: REG_empleado;
begin
  write('Nombre del archivo: ');
  readln(nombre);
  ...
  write('Ingresar num. empleado: ');
  readln(numEmpleado);

  read( a, r );
  while ( not Eof(a)) and ( r.nroEmpleado <> numEmpleado ) do
    read( a, r );

  if ( r.nroEmpleado = numEmpleado ) then begin
    // Guardo la posicion del empleado a eliminar
    pos:= FilePos(a) - 1;

    // Guardar la ultima posición del archivo
    ultimaPos:= FileSize(a) - 1 ;

    if ( pos <> ultimaPos ) then begin
      Seek( a, ultimaPos );
      read( a, r );
      Seek( a, pos );
      Write( a, r );
